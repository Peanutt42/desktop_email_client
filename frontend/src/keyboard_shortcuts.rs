...
use yew::{Component, Context, Html, KeyboardEvent, NodeRef, Properties};

use crate::views::focus_email_searchbar_input;

fn handle_keyboard_shortcut(e: &KeyboardEvent, email_searchbar_input_ref: &NodeRef) {
...
impl Component for KeyboardShortcutListener {
	...
	/// readds the event listener every rerender
	fn view(&self, ctx: &Context<Self>) -> Html {
		let email_searchbar_input_ref = ctx.props().email_searchbar_input_ref.clone();
		let new_event_listener = add_keyboard_shortcut_event_listener(email_searchbar_input_ref);
...
fn add_keyboard_shortcut_event_listener(email_searchbar_input_ref: NodeRef) -> EventListener {
	let window = window().unwrap();
	EventListener::new(&window, "keydown", move |e| {
		if let Some(e) = e.dyn_ref::<KeyboardEvent>() {
			handle_keyboard_shortcut(e, &email_searchbar_input_ref);
		}
	})
}
